*-------------------------------------------------------------------------------
* Title      : HEX DUMP Disassembler
* Written by : Trinh Ta and Leo Le
* Date       : 11/20/2018
* Description: This is the final project of CSS 422
*              This disassembler will take in the starting address
*              and ending address of the user, then will start 
*              disassemble the data within that specific section
*              into assembly code and output back to the user.
*              Each page will have a limit of offset in the 
*              memory, so to move forward, press ENTER to get
*              the next data page.
*              The program will go on until it reaches the end
*              of the ending address provided.
*-------------------------------------------------------------------------------
    ORG    $1000
* LOCATION TO STORE START_ADDRESS
START_ADDRESS EQU $2EEC
* LOCATION TO STORE END_ADDRESS
END_ADDRESS EQU $2EF0
* LOCATION TO STORE CURRENT 16 BITS OPCODE
OPCODE_ADDRESS EQU $2EF4
* LOCATION TO STORE CURRENT OFFSET FROM OPCODE OF A INSTRUCTION, FOR EXAMPLE:
*       MOVE.L #$500, D3 and the instruction(MOVE.L known data register as d3 and immediate data) location at 10000.
*       THIS OFFSET WILL HANDLE BY PRINT_REGISTER. IT WILL PRINT OUT #$500 AND ADD THE NEXT OFFSET FROM THE ORIGINAL OPCODE IN THIS LOCATION, 
*       THE OFFSET SHOULD SET = 2 WHEN FETCH_OPCODE 
OFFSET_OPC_ADDRES EQU $2EF8
* RETURN_ADDRESS USES FOR PRINTING AND RETURN VALUE FROM BRANCHES
RETURN_ADDRESS EQU $2EFA
* OUTPUT_ADDRESS STORES BUFFER FOR PRINTING
OUTPUT_ADDRESS EQU $3000
* LOCATION TO STORE STACK ADDRESS VALUE
STACK_ADDRESS EQU $3500

MAXIMUM_ROW EQU 29
START:
 
GET_USER_INPUT:
    JSR GET_INPUT_START
    JSR GET_INPUT_END
    MOVE.L (START_ADDRESS), D1 
    MOVE.L (END_ADDRESS), D2 
    CMP.L D1,D2 
    BLT START_GT_END
    BRA MAIN
START_GT_END:
    LEA INVALID_ADDRESS_SGE, A1 
    MOVE.B #13, D0 
    TRAP #15 
    BRA GET_USER_INPUT
MAIN:
    ; clear screen
    MOVE.W #$FF00, D1 
    MOVE.B #11, D0 
    TRAP #15
    CLR.B D7 ; use d7 for counter main loop
LOOP_START:
    MOVE.L (START_ADDRESS), D1 
    MOVE.L (END_ADDRESS), D2 
    CMP.L D1,D2 
    BMI END_MAIN 
    ; initilize stack address
    MOVE.L #STACK_ADDRESS, SP
    MOVE.L (START_ADDRESS), -(SP)
    MOVE.L #OPCODE_ADDRESS, -(SP)
    ; featch word opcode to opcode address
    JSR FETCH_OPCODES
    ; align stack address
    ADDQ.L #8, SP
    ; print out program counter
    JSR PRINT_PC
    ; pass opcode to jump table
    MOVE.L #OPCODE_ADDRESS, A0
    MOVE.W (A0), -(SP)
    JSR JUMP_TABLE
FINISH_OPCODE:
    CLR.L D0
    MOVE.L #$0, (RETURN_ADDRESS) ; SET ALL THE FLAG IN RETURN ADDRESS  TO ZERO 
    MOVE.W (OFFSET_OPC_ADDRES), D0
    ADD.L D0, (START_ADDRESS)
    JSR PRINT_NEW_LINE
    ADDQ.B #1, D7 
    CMPI.B #MAXIMUM_ROW, D7 
    BEQ CLR_SCREEN
    BRA LOOP_START

CLR_SCREEN:
    LEA PROMT_INPUT_CONTINUE, A1 
    MOVE.B #13, D0 
    TRAP #15 
    CLR.B D7
    MOVE.B #5, D0 
    TRAP #15 
    * is it enter character
    CMPI.B #$0D, D1 
    BNE END_MAIN
    BRA LOOP_START


END_MAIN:
    MOVE.B #9, D0
    TRAP #15
* get user input 
GET_INPUT_START:
    CLR.L D2 
    LEA PROMT_INPUT_START, A1
    MOVE.B #14, D0
    TRAP #15
    MOVEA.L #START_ADDRESS,A1
    MOVE.B #2, D0
    TRAP #15
    MOVE.L A1, -(SP)
    MOVE.L D1, -(SP)
    MOVE.L #8, -(SP)
    JSR ASCII_TO_HEX 
    ADD.L #12, SP 
    MOVE.W (RETURN_ADDRESS), D1 
    CMPI.B #ASCII_TO_HEX_RETURN_OK, D1 
    BNE RE_INPUT_START
    MOVE.L #RETURN_ADDRESS, A2 
    MOVE.L 2(A2), D2 
    CMPI.L #STACK_ADDRESS, D2 
    BLT GET_INPUT_START
    CLR.L D3
    MOVE.B D2, D3 
    DIVU.W #2, D3 
    SWAP D3 
    CMPI.B #0, D3 
    BNE RE_INPUT_START

    MOVE.L D2, (START_ADDRESS) 
    BRA END_GET_START_INPUT
RE_INPUT_START:
    LEA INVALID_ADDRESS, A1 
    MOVE.B #13, D0 
    TRAP #15
    BRA GET_INPUT_START
END_GET_START_INPUT:
    RTS   


GET_INPUT_END:
    CLR.L D2 
    LEA PROMT_INPUT_END, A1
    MOVE.B #14, D0
    TRAP #15
    MOVEA.L #END_ADDRESS,A1
    MOVE.B #2, D0
    TRAP #15
    MOVE.L A1, -(SP)
    MOVE.L D1, -(SP)
    MOVE.L #8, -(SP)
    JSR ASCII_TO_HEX 
    ADD.L #12, SP 
    MOVE.W (RETURN_ADDRESS), D1 
    CMPI.B #ASCII_TO_HEX_RETURN_OK, D1 
    BNE RE_INPUT_END 
    MOVE.L #RETURN_ADDRESS, A2
    MOVE.L 2(A2), D2 
    CMPI.L #STACK_ADDRESS, D2 
    BLT GET_INPUT_END
    CLR.L D3
    MOVE.B D2, D3 
    DIVU.W #2, D3 
    SWAP D3 
    CMPI.B #0, D3 
    BNE RE_INPUT_END
    MOVE.L D2, (END_ADDRESS)
    BRA END_GET_END_INPUT
RE_INPUT_END:
    LEA INVALID_ADDRESS, A1 
    MOVE.B #13, D0 
    TRAP #15
    BRA GET_INPUT_END
END_GET_END_INPUT:
    RTS


*--------------------------------------------------------------------
* PRINT_TAB 
PRINT_TAB: 
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.L #'    ', (A1)+
    MOVE.B #0, (A1)+
    MOVE.L #OUTPUT_ADDRESS, A1
END_PRINT_TAB:
    MOVE.B #14, D0
    TRAP #15
    RTS
*--------------------------------------------------------------------
* PRINT_COMMA
PRINT_COMMA:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.W #', ', (A1)+
    MOVE.B #0, (A1)
    SUBQ.L #2, A1
END_PRINT_COMMA:
    MOVE.B #14, D0
    TRAP #15
    RTS
*--------------------------------------------------------------------
* PRINT_DOLLAR_SIGN
PRINT_DOLLAR_SIGN:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.B #'$', (A1)
    MOVE.B #0, 1(A1) 
END_PRINT_DOLLAR_SIGN:
    MOVE.B #14, D0
    TRAP #15
    RTS
*--------------------------------------------------------------------
* PRINT_HASH_SIGN
PRINT_HASH_SIGN:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.B #'#', (A1)
    MOVE.B #0, 1(A1) 
END_PRINT_HASH_SIGN:
    MOVE.B #14, D0
    TRAP #15
    RTS

*--------------------------------------------------------------------
* PRINT A NEW LINE 
PRINT_NEW_LINE: 
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.B #CR, (A1)+ 
    MOVE.B #LF, (A1)+
    MOVE.B #0, (A1)+
END_PRINT_NEW_LINE:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.B #14, D0
    TRAP #15 
    RTS

*--------------------------------------------------------------------
* PRINT DATA TYPE FOR EXAMPLE: .B .W .L
* PARAMETERS: 
 *   1- (4(sp)): WHAT DATA TYPE (1 BYTE)
*       00 - byte
*       01 - word
*       10 - long
* NOTE:
*   A1 USE FOR PRINT
*   d1 USES TO STORE DATA TYPE OF PARAMETER ABOVE
PRINT_DATA:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.B 4(SP), D1
    CMPI.B #$00, D1
    BEQ PRINT_BYTE
    CMPI.B #$01, D1
    BEQ PRINT_WORD
    CMPI.B #$02, D1 
    BEQ PRINT_LONG
END_PRINT_DATA:
    MOVE.B #14, D0
    TRAP #15
    RTS 

PRINT_BYTE: 
    MOVE.W #'.B', (A1)
    MOVE.B #0, 2(A1)
    BRA END_PRINT_DATA
PRINT_WORD: 
    MOVE.W #'.W', (A1)
    MOVE.B #0, 2(A1)
    BRA END_PRINT_DATA
PRINT_LONG: 
    MOVE.W #'.L', (A1)
    MOVE.B #0, 2(A1)
    BRA END_PRINT_DATA

*--------------------------------------------------------------------
* PRINT CURRENT PROGRAM COUNTER IN HEX 
* WILL GET THE CURRENT PC IN START ADDRESS AND PRINT IT OUT
* PARAMETERS:
*   NONE
* NOTES: 
* A1 USE TO PRINT 
* D0 FOR TRAP AND CONVERSION 
PRINT_PC:
    MOVE.L (START_ADDRESS), D0 
    MOVE.L D0, -(SP)
    MOVE.L #HEX_TO_ASCII_LONG, -(SP)
    JSR PRINT_HEX 
    ADDQ.L #8, SP 
    JSR PRINT_TAB
    JSR PRINT_TAB 
    RTS 

CHECK_SUPPORT_REGISTER:
    MOVE.W #0, (RETURN_ADDRESS) ; reset return address 
    MOVE.W 4(SP), D1 
    MOVE.W D1,D4 
   ; check to make sure we dont support the register 
DISPLACEMENT_REGISTER EQU %00101000
ADDRESS_WITH_INDEX EQU %00110000
COUNTER_INDEX EQU %00111011 
COUNTER_DISPLACEMENT EQU %00111010
    AND.B #%00111000, D4 
    CMPI.B #DISPLACEMENT_REGISTER, D4
    BEQ UNSUPPORT_REGISTER
    CMPI.B #ADDRESS_WITH_INDEX, D4
    BEQ UNSUPPORT_REGISTER 
    MOVE D1, D4
    AND.B #%00111111,D4
    CMPI.B #COUNTER_INDEX, D4
    BEQ UNSUPPORT_REGISTER 
    CMPI.B #COUNTER_DISPLACEMENT, D4
    BEQ UNSUPPORT_REGISTER
    RTS

UNSUPPORT_OPCODE: 
    MOVE.L #RETURN_ADDRESS, A3
    JSR PRINT_UNSUPPORTED
    JSR PRINT_TAB 
    JSR PRINT_HASH_SIGN 
    JSR PRINT_DOLLAR_SIGN 
    CLR.L D1 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.L D1, -(SP) 
    MOVE.L #HEX_TO_ASCII_WORD, -(SP)
    JSR PRINT_HEX 
    ADDQ.L #8, SP 
    MOVE.W #C_UNSUPPORT_REGISTER, (A3)
    RTS 

UNSUPPORT_REGISTER:
C_UNSUPPORT_REGISTER EQU $2
    MOVE.L #RETURN_ADDRESS, A3
    JSR PRINT_UNSUPPORTED
    JSR PRINT_TAB 
    JSR PRINT_HASH_SIGN 
    JSR PRINT_DOLLAR_SIGN 
    CLR.L D1 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.L D1, -(SP) 
    MOVE.L #HEX_TO_ASCII_WORD, -(SP)
    JSR PRINT_HEX 
    ADDQ.L #8, SP 
    MOVE.W #C_UNSUPPORT_REGISTER, (A3)
    ADDQ.W #2, (OFFSET_OPC_ADDRES)
    BRA FINISH_OPCODE

*--------------------------------------------------------------------
* PRINT_REGISTER
* PARAMETERS: what register 1 byte 4 msb indicate dn(0) or an(1), 4 lsb for number of that register
* 1 - (6(sp)) 3 bits msb for mode, 3 bits lsb for which register
* 2 - (4(sp)) byte value for how many byte for immediate value
*      0 for a byte value
*      1 for word value
*      2 for long value 
* RETURN 2 IN RETURN ADDRESS IF THE MODE DOESNT SUPPORT 
* NOTE: 
*   D1 FOR STORE THE PARAMETER
*   D2 FOR STORE HOW MANY BYTE FOR IMMEDIATE VALUE, AND ABSOLUTE VALUE
PRINT_REGISTER:
IMMEDATE_VALUE EQU %00111100
   MOVE.W 6(SP), D1 
   
   MOVE.W D1,D2
   CMPI.B #%00111100, D1
   BEQ PRINT_IMMEDIATE
   CMPI.B #%00111000, D1
   BEQ PRINT_AB_SHORT
   CMPI.B #%00111001, D1
   BEQ PRINT_AB_LONG 

   MOVE.B D1,D2
   AND.B #%00111000, D2
   CMPI.B #%00010000, D2
   BEQ PRINT_OPEN

   MOVE.B D1,D2
   AND.B #%00111000, D2
   CMPI.B #%00011000, D2
   BEQ PRINT_OPEN

   MOVE.B D1,D2
   AND.B #%00111000, D2
   CMPI.B #%00100000, D2
   BEQ PRINT_MINUS_OPEN

CONTINUE_PRINT_REGISTER:
   MOVE.B D1,D2
   AND.B #%00111000, D2
   CMPI.B #%00000000, D2
   BEQ PRINT_D

   MOVE.B D1,D2
   AND.B #%00111000, D2
   CMPI.B #%00001000, D2
   BEQ PRINT_A 

CONTINUE_PRINT_RE_NUMBER:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.B D1,D2
    AND.B #%00000111, D2
    ADD.B #48, D2 
    MOVE.B D2, (A1) 
    MOVE.B #$0, 1(A1)
    MOVE.B #14, D0
    TRAP #15
DONE_PRINT_NUMBER:
   MOVE.B D1,D2
   AND.B #%00111000, D2
   CMPI.B #%00010000, D2
   BEQ PRINT_CLOSE

   MOVE.B D1,D2
   AND.B #%00111000, D2
   CMPI.B #%00011000, D2
   BEQ PRINT_CLOSE_PLUS

   MOVE.B D1,D2
   AND.B #%00111000, D2
   CMPI.B #%00100000, D2
   BEQ PRINT_CLOSE

END_PRINT_REGISTER:
    RTS

PRINT_IMMEDIATE:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.B #'#', (A1)
    MOVE.B #$0, 1(A1)
    MOVE.B #14, D0
    TRAP #15
    JSR PRINT_DOLLAR_SIGN
    MOVE.W (OFFSET_OPC_ADDRES),D0
    MOVE.W 4(SP), D2 ; HOW MANY BYTE
    CMPI.B #$00, D2
    BEQ PRINT_IMMEDIATE_BYTE ; THIS CASE NEVER HAPPEN BECAUSE IMMEDATE VALUE MINIMUM IS 16 bits
    CMPI.B #$01, D2
    BEQ PRINT_IMMEDIATE_WORD
    CMPI.B #$02, D2
    BEQ PRINT_IMMEDIATE_LONG
    BRA END_PRINT_REGISTER

PRINT_IMMEDIATE_BYTE:
    MOVE.L (START_ADDRESS), A2
    MOVE.W (A2, D0), D2 ; 
    MOVEM.L D0-D2/A0-A1, -(SP)
    MOVE.L D2, -(SP)
    MOVE.L #HEX_TO_ASCII_WORD, -(SP) 
    JSR PRINT_HEX
     ADDQ.L #$08, SP
    MOVEM.L (SP)+, D0-D2/A0-A1
    ADDQ.W #$2, D0
    MOVE.W D0, (OFFSET_OPC_ADDRES)
    BRA END_PRINT_REGISTER
PRINT_IMMEDIATE_WORD:
    MOVE.L (START_ADDRESS), A2
    MOVE.W (A2, D0), D2 ; 
    MOVEM.L D0-D2/A0-A1, -(SP)
    MOVE.L D2, -(SP)
    MOVE.L #HEX_TO_ASCII_WORD, -(SP) 
    JSR PRINT_HEX
    ADDQ.L #$08, SP
    MOVEM.L (SP)+, D0-D2/A0-A1
    ADDQ.W #$2, D0
    MOVE.W D0, (OFFSET_OPC_ADDRES)
    BRA END_PRINT_REGISTER
PRINT_IMMEDIATE_LONG:
    MOVE.W (OFFSET_OPC_ADDRES),D0
    MOVE.L (START_ADDRESS), A2
    MOVE.L (A2, D0), D2 ; 
    MOVEM.L D0-D2/A0-A1, -(SP)
    MOVE.L D2, -(SP)
    ; offset to get immediate value from opcode
    MOVE.W (OFFSET_OPC_ADDRES),D0
    MOVE.L #HEX_TO_ASCII_LONG, -(SP) 
    JSR PRINT_HEX
    ADDQ.L #$08, SP
    MOVEM.L (SP)+, D0-D2/A0-A1
    ADDQ.W #$4, D0
    MOVE.W D0, (OFFSET_OPC_ADDRES)
    BRA END_PRINT_REGISTER
PRINT_AB_SHORT:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.W #'($', (A1)
    MOVE.B #0, 2(A1)
    MOVE.B #14, D0
    TRAP #15
    ; offset to get immediate value from opcode
    MOVE.L (START_ADDRESS), A2
    MOVE.W (OFFSET_OPC_ADDRES),D0
    MOVE.W (A2, D0), D2 ; 
    MOVEM.L D0-D2/A0-A1, -(SP)
    MOVE.L D2, -(SP)
    MOVE.L #HEX_TO_ASCII_WORD, -(SP) 
    JSR PRINT_HEX
    ADDQ.L #$08, SP
    MOVEM.L (SP)+, D0-D2/A0-A1
    ADDQ.W #$2, D0
    MOVE.W D0, (OFFSET_OPC_ADDRES)
    ; print close parathesis with W
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.W #').', (A1)
    MOVE.B #'W',2(A1)
    MOVE.B #0, 3(A1)
    MOVE.B #14, D0
    TRAP #15
    BRA END_PRINT_REGISTER
PRINT_AB_LONG:
    ;print open parathesis
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.W #'($', (A1)
    MOVE.B #0, 2(A1)
    MOVE.B #14, D0
    TRAP #15
    ; print value in long
    MOVE.L (START_ADDRESS), A2
    ; offset to get value from opcode
    MOVE.W (OFFSET_OPC_ADDRES),D0
    MOVE.L (A2, D0), D2 ; 
    MOVEM.L D0-D2/A0-A1, -(SP)
    MOVE.L D2, -(SP)
    MOVE.L #HEX_TO_ASCII_LONG, -(SP) 
    JSR PRINT_HEX
    ADDQ.L #$08, SP
    MOVEM.L (SP)+, D0-D2/A0-A1
    ADDQ.W #$4, D0
    MOVE.W D0, (OFFSET_OPC_ADDRES) ; align offset 
    ; print close parathesis with W
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.W #').', (A1)
    MOVE.B #'L',2(A1)
    MOVE.B #0, 3(A1)
    MOVE.B #14, D0
    TRAP #15
    BRA END_PRINT_REGISTER
PRINT_OPEN:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.B #'(', (A1)
    MOVE.B #$0, 1(A1)
    MOVE.B #14, D0
    TRAP #15
    BRA PRINT_A
PRINT_MINUS_OPEN: 
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.W #'-(', (A1)
    MOVE.B #$0, 2(A1)
    MOVE.B #14, D0
    TRAP #15
    BRA PRINT_A
PRINT_CLOSE:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.B #')', (A1)
    MOVE.B #$0, 1(A1)
    MOVE.B #14, D0
    TRAP #15
    BRA END_PRINT_REGISTER
PRINT_CLOSE_PLUS:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.W #')+', (A1)
    MOVE.B #$0, 2(A1)
    MOVE.B #14, D0
    TRAP #15
    BRA END_PRINT_REGISTER
PRINT_D:
    MOVE.L #OUTPUT_ADDRESS, A1 
    MOVE.B #'D', (A1)
    MOVE.B #$0, 1(A1)
    MOVE.B #14, D0
    TRAP #15
    BRA CONTINUE_PRINT_RE_NUMBER
PRINT_A: 
    MOVE.L #OUTPUT_ADDRESS, A1 
    MOVE.B #'A', (A1)
    MOVE.B #$0, 1(A1)
    MOVE.B #14, D0
    TRAP #15
    BRA CONTINUE_PRINT_RE_NUMBER

*-----------------------------------------------------------
* PRINT_DISPLACEMENT FOR BRANCHES 
* PARAMETERS:
*   1- 4(SP): 1 BYTE FOR WHAT SIZE
*             0- FOR S 
*             1- FOR W
*  NOTE: D0 USE FOR STORE SIZE
*        A0 TO STORE PC 
*        D1 FOR 2 COMPLEMENT 
PRINT_DISPLACEMENT:
    MOVE.L (START_ADDRESS), A0
    MOVE.B 4(SP), D0 
    CMPI.B #0, D0 
    BNE PRINT_DISPLACEMENT_S
    BRA PRINT_DISPLACEMENT_M
PRINT_DISPLACEMENT_S:
    MOVE.L #OUTPUT_ADDRESS, A1 
    ; print .S 
    MOVE.W #'.S', (A1)
    MOVE.B #0, 2(A1) 
    MOVE.B #14, D0 
    TRAP #15
    JSR PRINT_TAB
    JSR PRINT_DOLLAR_SIGN
    ; check if it is negative number
    CLR.L D0
    MOVE.B 1(A0), D0 
    BMI PRINT_DISPLACEMENT_S_N
    ; else it is positive 
    MOVE.L A0, D1 
    ADDQ.L #2, D1
    ADD.L D0, D1 
    MOVE.L D1, -(SP)
    MOVE.L #HEX_TO_ASCII_LONG, -(SP)
    JSR PRINT_HEX
    ADDQ.L #8, SP
    RTS
PRINT_DISPLACEMENT_S_N:
    MOVE.L A0, D1 
    ADDQ.L #2, D1
    NEG.B D0 
    SUB.L D0, D1 
    MOVE.L D1, -(SP)
    MOVE.L #HEX_TO_ASCII_LONG, -(SP)
    JSR PRINT_HEX
    ADDQ.L #8, SP
    RTS

PRINT_DISPLACEMENT_M:
    MOVE.L #OUTPUT_ADDRESS, A1 
    ; print .S 
    MOVE.W #'.W', (A1)
    MOVE.B #0, 2(A1) 
    MOVE.B #14, D0 
    TRAP #15
    JSR PRINT_TAB
    JSR PRINT_DOLLAR_SIGN
    CLR.L D0
    MOVE.W 2(A0), D0
    BMI PRINT_DISPLACEMENT_M_N
    MOVE.L A0, D1 
    ADDQ.L #2, D1 
    ADD.L D0, D1
    MOVE.L D1, -(SP)
    MOVE.L #HEX_TO_ASCII_LONG, -(SP)
    JSR PRINT_HEX
    ADDQ.L #8, SP
    ADDQ.L #$2, (START_ADDRESS)
    RTS
PRINT_DISPLACEMENT_M_N:
    MOVE.L A0 , D1 
    ADDQ.L #2, D1 
    NEG.W D0 
    SUB.L D0, D1 
    MOVE.L D1, -(SP)
    MOVE.L #HEX_TO_ASCII_LONG, -(SP)
    JSR PRINT_HEX
    ADDQ.L #8, SP
    ADDQ.L #$2, (START_ADDRESS)
    RTS


    
    








*-----------------------------------------------------------
* PRINT HEX 
* PARAMETERS:
*   1- (8(SP)): VALUE IN HEX
*   2- (4(SP)): HOW MANY BYTES
* RETURN NULL
*-----------------------------------------------------------
PRINT_HEX:
    BRA HEX_TO_ASCII
GET_ASCII_FROM_HEX:
    MOVE.L #OUTPUT_ADDRESS, A1
    MOVE.B #14, D0
    TRAP #15
    MOVE.L #OUTPUT_ADDRESS, A1
END_PRINT_HEX:
    RTS
*-----------------------------------------------------------
* CONVERT HEX TO ASCII 
* PARAMATERS:
*   1- (8(SP)): VALUE IN HEX
*   2- (4(SP)): HOW MANY BYTES
* RETURN:
*   8 CHARS IN ASCII, STORE IN RETURN ADDRESS (a1)
* D0 STORES HEX VALUE
* D4 HOW MAY BYTES TO DISPLAY (2-byte 4-hex 8-long)
* D1 COUNT LOOP
* D2 CONVERT TO ASCII
* D3 USE TO CALCULATE SHIFT
HEX_TO_ASCII: 
    MOVE.L 8(SP), D0 
    MOVE.L 4(SP), D4
    CMPI #HEX_TO_ASCII_WORD, D4 
    BEQ HEX_TO_ASCII_SWAP
    CMPI #HEX_TO_ASCII_BYTE, D4
    BEQ HEX_TO_ASCII_BYTE
HEX2ASCII_DONE_CONVERT:
    CLR.B D1
    MOVE.L #OUTPUT_ADDRESS, A1
HEX_MASKING EQU $0000000F
HEX_TO_ASCII_BYTE EQU $02
HEX_TO_ASCII_WORD EQU $04
HEX_TO_ASCII_LONG EQU $08

HEX_TO_ASCII_LOOP:
    CMP.B D4, D1
    BEQ END_HEX_TO_ASCII ; branch if d1 > d4

    ROL.L #4, D0 ; shift left 4 bits to get a hex value
    MOVE.L D0,D3 ; copy the value to mask
    AND.L #HEX_MASKING, D3  ; mask

    CMPI.B #10, D3 ; compare masked value with 10 if less than than convert to ascii number
    BLT HEX_TO_ASCII_NUMBER ; let convert to number ascii

    CMPI.B #72, D3 ; compare masked value with F letter in ascii
    BLT HEX_TO_ASCII_LETTER

    BRA HEX_TO_ASCII_LOOP
HEX_TO_ASCII_NUMBER:
    ADD.B #48, D3  ; convert number to letter
    MOVE.B D3, (A1)+ ; push to return memory
    ADDQ.B #$1, D1 ; add for loop count
    BRA HEX_TO_ASCII_LOOP ; loop back
HEX_TO_ASCII_LETTER:
    ADD.B #55, D3 ; convert A-F in ascii
    MOVE.B D3, (A1)+ ; push to return memory
    ADDQ.B #$1, D1 ; add to for loop count
    BRA HEX_TO_ASCII_LOOP ;loop back
END_HEX_TO_ASCII:
    MOVE.B #0, (A1)+ ; add null terminated value
    BRA GET_ASCII_FROM_HEX
HEX_TO_ASCII_SWAP:
    SWAP D0
    BRA HEX2ASCII_DONE_CONVERT
   
HEX_TO_ASCII_ROR:
    ROR.L #$08, D0
    BRA HEX2ASCII_DONE_CONVERT

*-----------------------------------------------------------
* CHECK IF THE VALUE IN HEX IS ODD 
* PARAMETER;* 1(4(SP))- HEX VALUE (LONG)
* RETURN:
*   0: IF IT IS EVEN
*   1: IF IT IS ODD
* NOTE:
*   A0 IS FOR RETURN ADDRESS
*   D0 IS FOR HEX VALUE
CHECK_VALUE_IS_ODD:
VALUE_IS_EVEN EQU $0
VALUE_IS_ODD EQU $1
    LEA RETURN_ADDRESS, A0
    MOVE.L 4(SP), D0
    AND.L #$0000FFFF, D0
    DIVU.W #2, D0
    SWAP D0      
    CMPI.W #0, D0
    BEQ CHECK_VALUE_IS_ODD_FAILED
    MOVE.W #VALUE_IS_ODD, (A0)
    BRA END_CHECK_VALUE_IS_ODD
END_CHECK_VALUE_IS_ODD:   
    RTS

CHECK_VALUE_IS_ODD_FAILED:
    MOVE.W #VALUE_IS_EVEN, (A0)
    BRA END_CHECK_VALUE_IS_ODD

*-----------------------------------------------------------
* CONVERT ASCII TO HEX
* PARAMETERS:
*   1. LOCATION OF THE ASCII
*   2. LENGTH OF THE ASCII
*   3. MAXIMUM LENGTH (hex/ byte / long)
* RETURN:
* [0=FAILED](2BYTE) (BECAUSE LENGTH > MAXIMUM_LENGTH)
* [1=OK](2BYTE)[VALUE IN HEX(4BYTES)]
* [2= FAILED](2BYTE)(BEACUSE IT IS NOT A NUMBER IN HEX)
* NOTE:
*    D0 FOR LENGTH OF ASCII
*    D1 FOR LOOPING
; CURRENT INDEX
*    D2 FOR THE VALUE IN ASCII OF THE DATA
*    D3 FOR CONVERT FINAL RESULT
*    D4 FOR CALCULATE NUMBER OF BIT TO SHIFT
*    A0 LOCATION OF THE DATA   
*    A2 CURRENT POINTER AT CONVERT_HEX_RETURN_ADDRESS
ASCII_TO_HEX:
ASCII_TO_HEX_RETURN_OK EQU $1
    LEA RETURN_ADDRESS, A2
    CLR.W (A2)
    CLR.L 2(A2)
    MOVE.L 12(SP),A0
 ; LOCATION OF THE DATA
    MOVE.L 8(SP), D0  ; LENGTH OF ASCII
    MOVE.L 4(SP), D1  ; MAX LENGTH
    CMP.L D0,D1 ; CHECK IF THE LENGTH IS GREATER THAN MAX LENGTH
    BLT ASCII_TO_HEX_FAILED_0
    CLR.L D1 ; FOR LOOPING
ASCII_TO_HEX_LOOP:
   CMP.L D1, D0 ;COMPARE FOR LOOPING
   BEQ ASCII_TO_HEX_END_LOOP
   ; D2 USE FOR STORE DATA 
    MOVE.B (A0)+, D2
    CMPI.B #47, D2
ASCII_TO_HEX_CHECK_GT_0
    BGT ASCII_TO_HEX_CHECK_LT_9 ; GREATER THAN LET COMPARE WITH LESS THAN 9
    BRA ASCII_TO_HEX_FAILED_2 ;ERROR NOT A HEX VALUE
ASCII_TO_HEX_CHECK_LT_9:
    CMPI.B #58, D2 ; CHECK IF IT IS LESS THAN NUMBER '9'
    BLT ASCII_TO_HEX_CONVERT_NUMBER ; LET CONVERT TO NUMBER
    ; IF IT IT IS NOT GREATHER THAN ZERO AND LESS THAN 9 THAN CHECK A-F
    BRA ASCII_TO_HEX_CHECK_GT_A
ASCII_TO_HEX_CHECK_GT_A:
    CMPI.B #64, D2
    BGT ASCII_TO_HEX_CHECK_LT_F ; LET CHECK IF IT IS LESS THAN F
    BRA ASCII_TO_HEX_FAILED_2  ; OTHERWISE THE VALUE IS NOT IN HEX
ASCII_TO_HEX_CHECK_LT_F
    CMPI.B #71, D2
    BLT ASCII_TO_HEX_CONVERT_LETTER
    BRA ASCII_TO_HEX_FAILED_2
* CONVERT THE NUMBER '0' - '9' IN HEX AND PUT IT IN D4
ASCII_TO_HEX_CONVERT_NUMBER:
     MOVE.L D0,D4 ; MOVE NUMBER OF LENGTH ASCII TO D4
     SUB.B #48, D2 ; MINUS THE NUMBER FROM ASCII TO GET IN HEX
     SUB.L D1,D4 ; TOTAL LENGTH - CURRENT INDEX OF VALUE IN A0
     SUBQ #1, D4 ; SUB 1 TO FOR SHIFT
     MULU #4, D4 ; MULTIPLY NUMBER OF BITS TO SHIFT
     ASL.L D4, D2 ; SHIFT DATA
     ADD.L D2, 2(A2) ; ADD THE DATA TO CURRENT VALUE IN A2 
     CLR.L D2
     ADDQ #1, D1  ; INCRASE LOOP COUNT INDEX
     BRA ASCII_TO_HEX_LOOP
ASCII_TO_HEX_CONVERT_LETTER:
    MOVE.L D0, D4
    SUB.B #55, D2
    SUB.L D1,D4
    SUBQ #1, D4
    MULU #4, D4
    ASL.L D4,D2
    ADD.L D2, 2(A2)
    CLR.L D2 
    ADDQ #1, D1
    BRA ASCII_TO_HEX_LOOP
ASCII_TO_HEX_END_LOOP:
    MOVE.W #1, (A2)
    RTS
    
ASCII_TO_HEX_FAILED_0:
    MOVE.W #2, (A2)
    RTS
ASCII_TO_HEX_FAILED_2:
    MOVE.W #2, (A2)
    RTS 
* THIS METHOD USES TO INCRASE START ADDRESS TO N VALUE 
* PARAMETERS:
*   2 (4(SP)) LONG VALUE: HOW MANY OFFSET
* RETURN:
*   NULL
* NOTE:
*   A0 USE TO POINT TO THE ADDRESS
*   d1 USE TO MATH 
INCRASE_POINTER_START_ADDRESS: 
    MOVE.L 4(SP), D1
    ADD.L D1, (START_ADDRESS)
    RTS

* FETCH 16 BIT FROM START_ADDRESS
* PARAMTERS: 
*   1- (8(SP)): WHERE IS OPCODE LOCATION
*   2- (4(SP)): WHERE TO STORE THE RESULT
* RETURN:
*   NULL
* NOTE:
*     A0 USE TO STORE THE OPCODE LOCATION
*     A2 USES TO RETURN THE OPCODE
FETCH_OPCODES:
    MOVE.L 8(SP), A0 ; LOCATION
    MOVE.L 4(SP), A2 ; OPCODE WILL BE RETURNED
    MOVE.W (A0), A0
    MOVE.W A0,(A2)
    MOVE.W #$2, (OFFSET_OPC_ADDRES)
END_FETCH_OPCODES:
    RTS



*------------------------------------DECODER PROCESS------------------------------------------------------------------------------------------------------------------
* jump table uses to jump to decode subroutine
* PARAMATERS:
* 1-(4(Sp)) Word - opcode
* NOTE: 
*   D1 USES TO STORE OPCODE
JUMP_TABLE:
    MOVE.W 4(SP), D1
    JSR ORI
    JSR MOVEA 
    JSR MOVE
    JSR NEG
    JSR NOP
    JSR RTS
    JSR JSR
    JSR MOVEM 
    JSR LEA
    JSR DIVS
    JSR OR
    JSR SUBQ
    JSR SUB
    * JSR EOR
    JSR CMPI
    JSR BCLR
    JSR CMP
    JSR MULS
    JSR ADDA
    JSR ADD 
    JSR MOVEA 
    JSR MOVE
    JSR MOVEM 
    JSR BRA
    JSR BCC
    JSR ALRSD_REGISTER
    JSR ALRSD_DATA
; IF THE OPCODES DONT MATCH THE OPCODE ABOVE THAN PRINT DATA OPCODE
END_JUMP_TABLE:
    JSR UNSUPPORT_OPCODE

NOP:
* constant for NOP 
C_NOP EQU $4E71
    CMP.W #C_NOP, D1
    BEQ PRINT_NOP
    RTS

RTS:
* constant for RTS 
C_RTS EQU $4E75
    CMP.W #C_RTS, D1
    BEQ PRINT_RTS
    RTS

JSR:
JSR_OPCODE EQU %0100111010000000
    MOVE.W D1,D2 
* MASKING WITH UNIQUE OPCODE
    AND.W #%1111111111000000, D2
    CMP.W #JSR_OPCODE, D2
    BNE JSR_END 
    ; check to make sure we support this register 
    MOVE.W D1, D2 
    AND.B #%00111111, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER 
    ADDQ #2, SP 
    ; check return value
    *  from print_register to see if it is unsupported register 
    MOVE.W (RETURN_ADDRESS), D2 
    CMPI.W #C_UNSUPPORT_REGISTER, D2 ; IF EQUAL RETURN TO FETCH NEXT OPCODE 
    BEQ JSR_END
    ; done checking 
    JSR PRINT_JSR
* MASKING TO GET MODE AND REGISTER
    MOVE.B D1,D2
    AND.B #%00111111, D2
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP
    BRA FINISH_OPCODE
JSR_END:
    RTS

LEA:
    MOVE.W D1,D2 
    AND.W  #%1111000111000000, D2
    CMPI.W #%0100000111000000, D2
    BNE LEA_END

    ; CHECK TO MAKE SURE WE SUPPORT THE EA
    MOVE.W D1, D2 
    AND.B #%00111111, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ #2, SP 
    ; check return value
    *  from print_register to see if it is unsupported register 
    MOVE.W (RETURN_ADDRESS), D2 
    CMPI.W #C_UNSUPPORT_REGISTER, D2 ; IF EQUAL RETURN TO FETCH NEXT OPCODE 
    BEQ FINISH_OPCODE
    * END CHECKING
    JSR PRINT_LEA
    MOVE.B D1,D2 
    ; MASK TO GET MODE AND REGISTER
    AND.B #%0000000000111111, D2
    MOVEM.W D1, -(SP)
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #$4, SP
    MOVEM.W (SP)+, D1
    JSR PRINT_COMMA
    ; MASK TO GET DESTINATION REGISTER
    MOVE.W D1,D2
    AND.W #%0000111000000000, D2
    LSR.W #$08, D2
    LSR.W #$01, D2
    OR.B #%00001000, D2 ; OR TO PASS TO PRINT_REGISTER
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP) 
    JSR PRINT_REGISTER
    ADDQ.L #$04, SP
    BRA FINISH_OPCODE
LEA_END:
    RTS

ADDA:
C_ADDA EQU       %1101000011000000
ADDA_MASKING EQU %1111000011000000
ADDA_SIZE_MASKING EQU %0000000100000000
ADDA_AN_MASKING EQU %0000111000000000
    MOVE.W D1, D2
    AND.W #ADDA_MASKING, D2 ; GET PATTERN
    CMPI.W #C_ADDA, D2 ; COMPARE WITH THE PATTERN
    BNE ADDA_END 
    ; CHECK TO MAKE SURE WE SUPPORT THE EA
    MOVE.W D1, D2 
    AND.B #%00111111, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ #2, SP 
    ; check return value
    *  from print_register to see if it is unsupported register 
    MOVE.W (RETURN_ADDRESS), D2 
    CMPI.W #C_UNSUPPORT_REGISTER, D2 ; IF EQUAL RETURN TO FETCH NEXT OPCODE 
    BEQ FINISH_OPCODE
    JSR PRINT_ADDA
    MOVE.W D1, D2  ; START TO MASK SIZE 
    AND.W #ADDA_SIZE_MASKING,D2 
    LSR.W #8, D2 
    MOVE.W D2 , D6 ; FOR SIZE TO PRINT REGISTER 
    ADDQ #$1, D6
    MOVE.B D6 , -(SP)
    JSR PRINT_DATA
    ADDQ.L #$2, SP 
    JSR PRINT_TAB
    ; GET MODE AND REGISTER
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.B D1, D2 
    AND.B #%00111111, D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER 
    ADDQ.L #$04, SP 
    JSR PRINT_COMMA
    MOVE.W (OPCODE_ADDRESS), D1  ; get back the opcode just in case
    MOVE.W D1, D2
    AND.W #ADDA_AN_MASKING, D2
    LSR.W #8, D2 
    LSR.W #1, D2 
    OR.B #%00001000, D2 ; TO APPEND MODE FOR PRINT REGISTER 
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP)
    JSR PRINT_REGISTER 
    ADDQ.L #$4, SP 
    BRA FINISH_OPCODE
ADDA_END:
    RTS

ADD: 
C_ADD EQU           %1101000000000000 ; PATTERN
C_ADD_MASKING EQU   %1111000000000000 ; MASKING TO CHECK PATTERN
C_ADD_EA EQU        %00111111 ; EFFECTIVE ADDRESS MASKING
C_ADD_REGISTER EQU  %0000111000000000 ; WHICH REGISTER
C_ADD_SIZE EQU      %0000000011000000  ; SIZE 
C_ADD_DIRECTION EQU %0000000100000000
C_ADD_TO_D EQU      %0000000000000000
C_ADD_FROM_D EQU    %0000000100000000
    MOVE.W D1, D2 
    ; CHECK PATTERN 
    AND.W #C_ADD_MASKING, D2 
    CMPI.W #C_ADD, D2 
    BNE ADD_END 
    ; CHECK SUPPORT REGISTER 
    MOVE.W D1, D2 
    AND.B #%00111111, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ #2, SP 
    ; check return value
    *  from print_register to see if it is unsupported register 
    MOVE.W (RETURN_ADDRESS), D2 
    CMPI.W #C_UNSUPPORT_REGISTER, D2 ; IF EQUAL RETURN TO FETCH NEXT OPCODE 
    BEQ FINISH_OPCODE
    
    JSR PRINT_ADD 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #C_ADD_SIZE, D2 
    LSR.W #6, D2
    MOVE.B D2, D6 ; SAVE FOR IMMEDIATE VALUE AND SIZE OF AN OPCODE  

    MOVE.B D2, -(SP)
    JSR PRINT_DATA
    ADDQ.L #$02, SP 
    JSR PRINT_TAB 
    ; CHECK DIRECTION
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2  
    AND.W #C_ADD_DIRECTION, D2 
    CMPI.W #C_ADD_TO_D, D2 
    BEQ ADD_TO_D ; DN <EA> -> DN
    ; otherwise from d 
ADD_FROM_D:
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #C_ADD_REGISTER, D2 
    LSR.L #8, D2 
    LSR.L #1, D2 
    OR.B #%00000000, D2
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP)
    JSR PRINT_REGISTER 
    ADDQ.L #4, D2 
    JSR PRINT_COMMA 
    
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.B #C_ADD_EA, D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE
    
ADD_TO_D:
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    ; GET SOURCE REGISTER 
    AND.B #C_ADD_EA, D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    JSR PRINT_COMMA 
    ; PRINT DATA REGISTER 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #C_ADD_REGISTER, D2 
    LSR.W #8, D2 
    LSR.W #1, D2 
    ; APPEND MODE TO D2 FOR PRINT_REGISTER
    OR.B #%00000000, D2 
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE
ADD_END:
    RTS

MOVEA: 
C_MOVEA_PATTERN1 EQU  %0001000001000000
C_MOVEA_PATTERN2 EQU  %0011000001000000
C_MOVEA_PATTERN3 EQU  %0010000001000000
C_MOVEA_P_MASKING EQU %1111000111000000
C_MOVEA_SIZE      EQU %0011000000000000
C_MOVEA_AN        EQU %0000111000000000
C_MOVEA_REGISTER  EQU %00111111
    MOVE.W D1, D2 
    ANDI.W #C_MOVEA_P_MASKING, D2 
    CMPI.W #C_MOVEA_PATTERN1, D2 
    BEQ MOVEA_REGISTER_OK
    CMPI.W #C_MOVEA_PATTERN2, D2 
    BEQ MOVEA_REGISTER_OK
    CMPI.W #C_MOVEA_PATTERN3, D2 
    BEQ MOVEA_REGISTER_OK
    RTS
MOVEA_REGISTER_OK:
    ; CHECK SUPPORT THE REGISTER 
    MOVE.W D1, D2 
    AND.B #C_MOVEA_REGISTER, D2 
    MOVE.W D2, -(SP) 
    JSR CHECK_SUPPORT_REGISTER
    ADDQ.L #2, SP 
    MOVE.W (RETURN_ADDRESS), D2 
    CMPI.W #C_UNSUPPORT_REGISTER, D2 
    BEQ FINISH_OPCODE
    ; DONE CHECKING, NOW DECODE 
    JSR PRINT_MOVEA 
    ; GET SIZE 
    MOVE.W D1, D2 
    AND.W #C_MOVEA_SIZE, D2 
    ; SHIFT TO LSB 
    LSR.W #8, D2 
    LSR.W #4, D2 
    CMPI.B #%00000001, D2
    BEQ CONVERT_OPMODE_SIZE_B
    CMPI.B #%00000011, D2 
    BEQ CONVERT_OPMODE_SIZE_W
    BRA MOVEA_DONE_CONVERT
CONVERT_OPMODE_SIZE_B:
    SUBQ.B #1, D2 
    BRA MOVEA_DONE_CONVERT
CONVERT_OPMODE_SIZE_W: 
    SUBQ.B #2, D2 
    BRA MOVEA_DONE_CONVERT

MOVEA_DONE_CONVERT:
    MOVE.W D2, D6 ; COPY THE SIZE FOR LATTER USE 
    MOVE.B D6, -(SP)
    JSR PRINT_DATA 
    ADDQ.L #2, SP 
    JSR PRINT_TAB
    ; GET SOURCE REGISTER 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.B #C_MOVEA_REGISTER,D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    JSR PRINT_COMMA

    MOVE.W (OPCODE_ADDRESS), D1 ; GET BACK THE OPCODE  
    MOVE.W D1, D2 ; MOVE TO D2 FOR LATER USE, NO NEED TO GET FROM MEMORY ANYMORE
    AND.W #C_MOVEA_AN, D2 
    LSR.W #8, D2 
    LSR.W #1, D2 
    ; ADD MODE FOR PRINT_REGISTER 
    ORI.B #%00001000, D2 
    MOVE.W D2, -(SP)
    MOVE.W #0, -(SP)
    JSR PRINT_REGISTER ; PRINT AN 
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE   

MOVEA_END: 
    RTS 

MOVE: 
C_MOVE_PATTERN EQU   %0001000000000000
C_MOVE_PATTERN1 EQU  %0011000000000000
C_MOVE_PATTERN2 EQU  %0010000000000000
C_MOVE_MASKING EQU   %1111000000000000
C_MOVE_SIZE EQU      %0011000000000000
C_MOVE_DES_XN EQU    %0000111000000000
C_MOVE_DES_MOD EQU   %0000000111000000
C_MOVE_SRC_RE EQU    %00111111 
    MOVE.W D1, D2 
    AND.W #C_MOVE_MASKING, D2 
    CMPI.W #C_MOVE_PATTERN, D2 
    BEQ MOVE_MATCH_PATTERN 
    CMPI.W #C_MOVE_PATTERN1, D2 
    BEQ MOVE_MATCH_PATTERN 
    CMPI.W #C_MOVE_PATTERN2, D2 
    BEQ MOVE_MATCH_PATTERN
MOVE_END:
    RTS
MOVE_MATCH_PATTERN: 
    MOVE.W D1, D2 
    ; CHECK IF WE SUPPORT THE REGISTER 
    ; SOURCE REGISTER 
    AND.B #C_MOVE_SRC_RE, D2
    MOVE.W D2, A5 ; COPY FOR LATER USE
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER 
    ADDQ.L #2, SP 
    CMPI.W #C_UNSUPPORT_REGISTER, (RETURN_ADDRESS)
    BEQ FINISH_OPCODE
    ; DES REGISTER 
    ;GET XN 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1,D2 
    AND.W #C_MOVE_DES_XN, D2 
    LSR.W #6, D2 
    AND.W #C_MOVE_DES_MOD, D1 
    OR.W D1, D2 
    LSR.W #3, D2 
    MOVE.W D2, A4 ; COPY FOR LATER USE 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ.L #2, SP 
    CMPI.W #CHECK_SUPPORT_REGISTER, (RETURN_ADDRESS)
    BEQ FINISH_OPCODE
    ; EVERYTHING IS OK 
    
    JSR PRINT_MOVE
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #C_MOVE_SIZE, D2 
    LSR.W #8, D2 
    LSR.B #4, D2 
    CMPI.B #1, D2 
    BEQ MOVE_OPMODE_BYTE
    CMPI.B #3, D2 
    BEQ MOVE_OPMODE_WORD
    BRA MOVE_DONE_CONVERT
MOVE_OPMODE_BYTE:
    SUBQ.B #1, D2
    BRA MOVE_DONE_CONVERT
MOVE_OPMODE_WORD:
    SUBQ.B #2, D2 
    BRA MOVE_DONE_CONVERT
MOVE_DONE_CONVERT:
    MOVE.W D2, D6 ; COPY THE SIZE FOR LATER USE
    MOVE.B D6, -(SP) 
    JSR PRINT_DATA 
    ADDQ.L #2, SP
    JSR PRINT_TAB 
    ; PRINT SOURCE
    MOVE.W A5, -(SP) ; MODE-REGISTER FOR SOURCE
    MOVE.W D6, -(SP) ; SIZE 
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    ; PRINT DES 
    JSR PRINT_COMMA 
    MOVE.W A4, -(SP) ; MODE-REGISTER FOR DES 
    MOVE.W D6, -(SP) ; SIZE 
    JSR PRINT_REGISTER 
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE 

MOVEM: 
C_MOVEM_PATTERN EQU     %0100100010000000
C_MOVEM_MASKING EQU     %1111101110000000
C_MOVEM_DIR EQU         %0000010000000000
C_MOVEM_TO_MEM EQU      %0000000000000000
C_MOVEM_TO_REGISTER   EQU      %0000010000000000
C_MOVEM_SIZE EQU        %0000000001000000
C_MOVEM_REGISTER EQU    %00111111
    MOVE.W D1, D2 
    AND.W #C_MOVEM_MASKING, D2 
    CMPI.W #C_MOVEM_PATTERN, D2 
    BNE MOVEM_END 
    ; CHECK IF THE VALID REGISTER 
    AND.W #C_MOVEM_REGISTER, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ.L #2, SP 
    CMPI.W #C_UNSUPPORT_REGISTER,(RETURN_ADDRESS)
    BEQ FINISH_OPCODE

    JSR PRINT_MOVEM 
    ; print size 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #C_MOVEM_SIZE, D2 
    LSR.W #6, D2 
    ADD.B #1, D2 ; convert to Size for print_register
    MOVE.W D2, A6 ; save to for later use 
    MOVE.B D2, -(SP) 
    JSR PRINT_DATA 
    ADDQ.L #2, SP 
    JSR PRINT_TAB
    ; GET DIRECTION
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #C_MOVEM_DIR, D2 
    CMPI.W #C_MOVEM_TO_MEM, D2 
    BEQ MOVEM_TO_MEM 
    CMPI.W #C_MOVEM_TO_REGISTER, D2 
    BEQ MOVEM_TO_REGISTER
MOVEM_TO_MEM:
    ; CHECK TO SEE IF IT IS PREDECREMENT 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    MOVE.B #0, D4 ; FOR LOOP 
    AND.B  #%00111000, D2 
    CMPI.B #%00100000, D2 
    BEQ MTM_PRED_LOOP
    BRA MTM_POST_LOOP
; PREDECREMENT 
MTM_PRED_LOOP:
    ADDQ.B #1, D4 
    CMPI.B #16, D4 
    BEQ MTM_PRED_LOOP_END 
    MOVE.L #START_ADDRESS, A0 
    MOVE.L (A0), A0 
    MOVE.W 2(A0), D2 ; THIS IS WORD REGISTERS 
    ROL.W D4, D2 
IS_ON EQU %00000001
    AND.B #IS_ON, D2 
    CMPI.B #IS_ON, D2 
    BNE MTM_PRED_LOOP
    ; OK LET PRINT 
    CMPI.B #8, D4 
    BGT MOVEM_PRINT_A ; TIME TO PRINT An
    BRA MOVEM_PRINT_D
MOVEM_PRINT_A: 
    MOVE.W D4, D5 
    SUBQ.B #1, D5 
    AND.B #%00000111, D5 
    OR.B  #%00001000, D5 
    MOVE.W D5, -(SP)
    MOVE.W #0, -(SP) 
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA DONE_PRINT_MOVEM_M
MOVEM_PRINT_D:
    MOVE.W D4, D5 
    SUBQ.B #1, D5 
    AND.B #%00000111, D5 
    MOVE.W D5, -(SP)
    MOVE.W #0, -(SP) 
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA DONE_PRINT_MOVEM_M

DONE_PRINT_MOVEM_M:
    ; PRINT / 
    MOVE.L #OUTPUT_ADDRESS, A1 
    MOVE.B #'/', (A1) 
    MOVE.B #0, 1(A1)
    MOVE.B #14, D0 
    TRAP #15
    BRA MTM_PRED_LOOP
  
    
MTM_PRED_LOOP_END:
    MOVE.L #OPCODE_ADDRESS,A0 
    ADDQ.W #2, 2(A0)
    JSR PRINT_COMMA 
    ; PRINT_REGISTER FOR DESTINATION 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1,D2 
    AND.B #C_MOVEM_REGISTER, D2 
    MOVE.W D2, -(SP)
    MOVE.W A6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP
    BRA FINISH_OPCODE

 MTM_POST_LOOP:
    CMPI.B #15, D4 
    BEQ MTM_POST_LOOP_END 
    MOVE.L #START_ADDRESS, A0 
    MOVE.L (A0), A0 
    MOVE.W 2(A0), D2 ; THIS IS WORD REGISTERS 
    ROR.W D4, D2 
    ADDQ.B #1, D4 
IS_ON EQU %00000001
    AND.B #IS_ON, D2 
    CMPI.B #IS_ON, D2 
    BNE MTM_POST_LOOP
    ; OK LET PRINT 
    CMPI.B #8, D4 
    BGT MOVEM_PPRINT_A ; TIME TO PRINT An
    BRA MOVEM_PPRINT_D
DONE_PRINT_MOVEM_P:
    MOVE.L #OUTPUT_ADDRESS, A1 
    MOVE.B #'/', (A1) 
    MOVE.B #0, 1(A1)
    MOVE.B #14, D0 
    TRAP #15
    BRA MTM_POST_LOOP

 MTM_POST_LOOP_END:
    MOVE.L #OPCODE_ADDRESS,A0 
    ADDQ.W #2, 2(A0)
    JSR PRINT_COMMA 
    ; PRINT_REGISTER FOR DESTINATION 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1,D2 
    AND.B #C_MOVEM_REGISTER, D2 
    MOVE.W D2, -(SP)
    MOVE.W A6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP
    BRA FINISH_OPCODE 
MOVEM_PPRINT_A: 
    MOVE.W D4, D5 
    SUBQ.B #1, D5 
    AND.B #%00000111, D5 
    OR.B  #%00001000, D5 
    MOVE.W D5, -(SP)
    MOVE.W #0, -(SP) 
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA DONE_PRINT_MOVEM_P
MOVEM_PPRINT_D:
    MOVE.W D4, D5 
    SUBQ.B #1, D5 
    AND.B #%00000111, D5 
    MOVE.W D5, -(SP)
    MOVE.W #0, -(SP) 
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 

    BRA DONE_PRINT_MOVEM_P

MOVEM_TO_REGISTER:
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1,D2 
    AND.B #C_MOVEM_REGISTER, D2 
    MOVE.W D2, -(SP) 
    MOVE.W A6, -(SP) 
    ADDQ.W #2, (OFFSET_OPC_ADDRES)
    JSR PRINT_REGISTER
    SUBQ.W #2, (OFFSET_OPC_ADDRES)
    ADDQ.L #4, SP 
    JSR PRINT_COMMA
    MOVE.B #0, D4 
MOVEM_TO_R_LOOP:
    CMPI.B #15, D4 
    BEQ MTR_POST_LOOP_END 
    MOVE.L #START_ADDRESS, A0 
    MOVE.L (A0), A0 
    MOVE.W 2(A0), D2 ; THIS IS WORD REGISTERS 
    ROR.W D4, D2 
    ADDQ.B #1, D4 
IS_ON EQU %00000001
    AND.B #IS_ON, D2 
    CMPI.B #IS_ON, D2 
    BNE MOVEM_TO_R_LOOP
    ; OK LET PRINT 
    CMPI.B #8, D4 
    BGT MOVEM_RPRINT_A ; TIME TO PRINT An
    BRA MOVEM_RPRINT_D
DONE_PRINT_MOVEM_R:
    MOVE.L #OUTPUT_ADDRESS, A1 
    MOVE.B #'/', (A1) 
    MOVE.B #0, 1(A1)
    MOVE.B #14, D0 
    TRAP #15
    BRA MOVEM_TO_R_LOOP
MOVEM_RPRINT_A: 
    MOVE.W D4, D5 
    SUBQ.B #1, D5 
    AND.B #%00000111, D5 
    OR.B  #%00001000, D5 
    MOVE.W D5, -(SP)
    MOVE.W #0, -(SP) 
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA DONE_PRINT_MOVEM_R
MOVEM_RPRINT_D:
    MOVE.W D4, D5 
    SUBQ.B #1, D5 
    AND.B #%00000111, D5 
    MOVE.W D5, -(SP)
    MOVE.W #0, -(SP) 
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA DONE_PRINT_MOVEM_R
    
MTR_POST_LOOP_END:
    MOVE.L #OPCODE_ADDRESS,A0 
    ADDQ.W #2, 2(A0)
    BRA FINISH_OPCODE 

MOVEM_END:
    RTS 




* ------------ LEO ------------ *          * ------------ LEO ------------ *          * ------------ LEO ------------ *
NEG:
* CONSTANT FOR NEG OPCODE
NEG_OPCODE EQU %0100010000000000
NEG_M      EQU %1111111100000000
NEG_REG_M  EQU %0000000000111111
NEG_SIZE_M EQU %0000000011000000
    MOVE.W D1, D2
* MASKING WITH NEG UNIQUE CODE
    AND.W #NEG_M, D2
    CMP.W #NEG_OPCODE, D2
    BNE NEG_END

* CHECK TO MAKE SURE WE SUPPORT THE EA
    MOVE.W D1, D2 
    AND.B #NEG_REG_M, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ #2, SP 
* from print_register to see if it is unsupported register 
    MOVE.W (RETURN_ADDRESS), D2 
    CMPI.W #C_UNSUPPORT_REGISTER, D2 ; IF EQUAL RETURN TO FETCH NEXT OPCODE 
    BEQ FINISH_OPCODE

    JSR PRINT_NEG
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2
* MASKING TO GET NEG SIZE
    AND.W #NEG_SIZE_M, D2
    ASR.W #6, D2
    MOVE.B D2 , D6 * COPY SHIFTED SIZE FOR PRINT REGISTER 
    MOVE.B D6, -(SP)
    JSR PRINT_DATA
    ADDQ.L #2, SP
    JSR PRINT_TAB

    MOVE.W (OPCODE_ADDRESS), D1     
    MOVE.W D1, D2
* MASKING TO GET MODE AND REGISTER
    AND.W #NEG_REG_M, D2
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #$04, SP
    BRA FINISH_OPCODE
NEG_END: 
    RTS

BRA:
* CONSTANT FOR BRA OPCODE
BRA_OPCODE EQU %0110000000000000
BRA_M      EQU %1111111100000000
BRA_SIZE_M EQU %0000000011111111
    MOVE.W D1, D2
* MASKING WITH BRA UNIQUE CODE
    AND.W #BRA_M, D2
    CMP.W #BRA_OPCODE, D2
    BNE BRA_END
    ; displacement 
    JSR PRINT_BRA
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2
    MOVE.W D1, D3
* MASKING TO GET SIZE OF BRA
    AND.B #BRA_SIZE_M, D2 
    CMP.B #0, D2
    MOVE.B D2, -(SP)
    JSR PRINT_DISPLACEMENT
    ADDQ.L #2, SP
    BRA FINISH_OPCODE
BRA_END:
    RTS

* EOR and OR
OR:
* CONSTANT FOR OR OPCODE
OR_PATTERN EQU  %1000000000000000
OR_PATTERN1 EQU %1000000001000000
OR_PATTERN2 EQU %1000000010000000
OR_PATTERN3 EQU %1000000100000000
OR_PATTERN4 EQU %1000000101000000
OR_PATTERN5 EQU %1000000110000000
; EOR PATTERN 
EOR_PATTERN EQU  %1011000100000000
EOR_PATTERN1 EQU %1011000101000000
EOR_PATTERN2 EQU %1011000110000000
OR_M      EQU   %1111000111000000
OR_DIR    EQU   %0000000100000000
OR_DN    EQU    %0000111000000000 
OR_REGISTER EQU %00111111
OR_SIZE_M EQU   %11000000
C_OR_DN     EQU $00
C_OR_EA     EQU $01
    MOVE.W D1, D2
* MASKING WITH OR UNIQUE CODE
    ANDI.W #OR_M, D2
    CMP.W #OR_PATTERN, D2
    BEQ OR_MATCH
    CMP.W #OR_PATTERN1, D2
    BEQ OR_MATCH 
    CMP.W #OR_PATTERN2, D2
    BEQ OR_MATCH
    CMP.W #OR_PATTERN3, D2
    BEQ OR_MATCH
    CMP.W #OR_PATTERN4, D2
    BEQ OR_MATCH 
    CMP.W #OR_PATTERN5, D2
    BEQ OR_MATCH
    CMP.W #EOR_PATTERN, D2
    BEQ EOR_MATCH
    CMP.W #EOR_PATTERN1, D2
    BEQ EOR_MATCH 
    CMP.W #EOR_PATTERN2, D2
    BEQ EOR_MATCH
    BRA OR_END
EOR_MATCH:
    MOVE.W D1, D2 
    ANDI.B #OR_REGISTER, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ.L #2, SP 
    JSR PRINT_EOR
    BRA OR_DONE_MATCH
OR_MATCH: * check to make sure we support this register
    MOVE.W D1, D2 
    ANDI.B #OR_REGISTER, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ.L #2, SP 
    JSR PRINT_OR
OR_DONE_MATCH:
    MOVE.W (OPCODE_ADDRESS), D2 
    ANDI.B #OR_SIZE_M, D2 
    LSR.B #6, D2 
    MOVE.B D2, D6 ; SAVE FOR LATER 
    MOVE.B D2, -(SP) 
    JSR PRINT_DATA 
    ADDQ.L #2, SP 
    JSR PRINT_TAB
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    ANDI.W #OR_DIR, D2 
    LSR.W #8, D2 
    CMPI.B #C_OR_DN, D2 
    BEQ OR_TO_DN 
    ;OTERHWISE THE DESTINATION IS EA 
OR_TO_EA:
    ; PRINT DN AS SOURCE 
    MOVE.W D1, D2 
    ANDI.W #OR_DN, D2 
    LSR.W #8, D2 
    LSR.B #1, D2 
    MOVE.W D2, -(SP)
    MOVE.W #0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    ; PRINT EA AS DESTINATION
    JSR PRINT_COMMA 
    MOVE.W (OPCODE_ADDRESS), D2 
    ANDI.B #OR_REGISTER, D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER 
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE
OR_TO_DN:
    ; PRINT SOURCE 
    MOVE.W D1, D2 
    ANDI.B #OR_REGISTER, D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    ; PRINT DESTINATION 
    JSR PRINT_COMMA
    MOVE.W (OPCODE_ADDRESS), D2 
    ANDI.W #OR_DN, D2
    LSR.W #8, D2 
    LSR.B #1, D2 
    MOVE.W D2, -(SP)
    MOVE.W #0, -(SP)
    JSR PRINT_REGISTER
    BRA FINISH_OPCODE
OR_END:
    RTS

ORI:
* CONSTANT FOR ORI OPCODE
ORI_OPCODE EQU %0000000000000000
ORI_M      EQU %1111111100000000
ORI_REG_M  EQU %00111111
ORI_SIZE_M EQU %0000000011000000
    MOVE.W D1, D2
* MASKING WITH ORI UNIQUE CODE
    AND.W #ORI_M, D2
    CMP.W #ORI_OPCODE, D2
    BNE ORI_END

* CHECK TO MAKE SURE WE SUPPORT THE EA
    MOVE.W D1, D2 
    AND.B #ORI_REG_M, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ #2, SP 

    JSR PRINT_ORI
    MOVE.W (OPCODE_ADDRESS), D1
    MOVE.W D1, D2
* MASKING TO GET SIZE OF ORI
    AND.W #ORI_SIZE_M, D2 
    ASR.W #6, D2
    MOVE.B D2 , D6 * COPY SHIFTED SIZE FOR PRINT REGISTER 
    MOVE.B D6, -(SP)
    JSR PRINT_DATA
    ADDQ.L #$02, SP
    JSR PRINT_TAB
 
    * MOVE.W (OPCODE_ADDRESS), D1     
    * MOVE.W D1, D2
    ; print immediate value
    MOVE.W #IMMEDATE_VALUE, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    JSR PRINT_COMMA
    ; print effective register 
    MOVE.W (OPCODE_ADDRESS), D1 
    ANDI.B #ORI_REG_M, D1 
    MOVE.W D1, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    BRA FINISH_OPCODE
ORI_END:
    RTS

* EOR:
* * CONSTANT FEOR EOR OPCODE
* EOR_OPCODE EQU %1011000100000000
* EOR_M      EQU %1111000100000000
* EOR_DREG_M EQU %0000111000000000
* EOR_SIZE_M EQU %0000000011000000
* EOR_REG_M  EQU %0000000000111111
* EOR_DIREC  EQU %0000000100000000

*     MOVE.W D1, D2
* * MASKING WITH EOR UNIQUE CODE
*     AND.W #EOR_M, D2
*     CMP.W #EOR_OPCODE, D2
*     BNE EOR_END

* * CHECK TO MAKE SURE WE SUPPEORT THE EA
*     MOVE.W D1, D2 
*     AND.B #EOR_REG_M, D2 
*     MOVE.W D2, -(SP)
*     JSR CHECK_SUPPORT_REGISTER
*     ADDQ #2, SP 
* * from print_register to see if it is unsuppEorted register 
*     MOVE.W (RETURN_ADDRESS), D2 
*     CMPI.W #C_UNSUPPORT_REGISTER, D2 ; IF EQUAL RETURN TO FETCH NEXT OPCODE 
*     BEQ FINISH_OPCODE
*     JSR PRINT_EOR
*     MOVE.W (OPCODE_ADDRESS), D1
*     MOVE.W D1, D2
* * MASKING TO GET SIZE OF EOR
*     AND.W #EOR_SIZE_M, D2
*     ASR.W #6, D2
*     MOVE.B D2, D6
*     MOVE.B D2, -(SP)
*     JSR PRINT_DATA
*     ADDQ.L #$02, SP 
*     JSR PRINT_TAB

* * <EA> EOR DN -> <EA>
*     MOVE.W (OPCODE_ADDRESS), D1 
*     MOVE.W D1, D2 
*     AND.W #EOR_DREG_M, D2 
*     LSR.L #8, D2 
*     LSR.L #1, D2 
*     OR.B #%00000000, D2
*     MOVE.W D2, -(SP)
*     MOVE.W #$0, -(SP)
*     JSR PRINT_REGISTER 
*     ADDQ.L #4, D2 
*     JSR PRINT_COMMA 
    
*     MOVE.W (OPCODE_ADDRESS), D1 
*     MOVE.W D1, D2 
*     AND.B #EOR_REG_M, D2 
*     MOVE.W D2, -(SP)
*     MOVE.W D6, -(SP)
*     JSR PRINT_REGISTER
*     ADDQ.L #4, SP 
*     BRA FINISH_OPCODE

* EOR_END:
*     RTS

SUB:
* CONSTANT FOR SUB OPCODE
SUB_OPCODE EQU %1001000000000000
SUB_M      EQU %1111000000000000
SUB_DREG_M EQU %0000111000000000
SUB_SIZE_M EQU %0000000011000000
SUB_REG_M  EQU %0000000000111111
SUB_DIREC  EQU %0000000100000000

    MOVE.W D1, D2
* MASKING WITH SUB UNIQUE CODE
    AND.W #SUB_M, D2
    CMPI.W #SUB_OPCODE, D2
    BNE SUB_END

* CHECK TO MAKE SURE WE SUPPORT THE EA
    MOVE.W D1, D2 
    AND.B #SUB_REG_M, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ.L #2, SP 
* from print_register to see if it is unsupported register 
    MOVE.W (RETURN_ADDRESS), D2 
    CMPI.W #C_UNSUPPORT_REGISTER, D2 ; IF EQUAL RETURN TO FETCH NEXT OPCODE 
    BEQ FINISH_OPCODE
    JSR PRINT_SUB
    MOVE.W (OPCODE_ADDRESS), D1
    MOVE.W D1, D2
* MASKING TO GET SIZE OF SUB
    AND.W #SUB_SIZE_M, D2
    ASR.W #6, D2
    MOVE.B D2, D6
    MOVE.B D2, -(SP)
    JSR PRINT_DATA
    ADDQ.L #$02, SP 
    JSR PRINT_TAB
* MASKING TO GET DIRECTION OF THIS SUB
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2  
    AND.W #SUB_DIREC, D2 
    CMPI.W #SUB_DIREC, D2 
    BNE SUB_TO_D  * DN - <EA> -> DN

SUB_TO_EA:  * <EA> - DN -> <EA>
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #SUB_DREG_M, D2 
    LSR.L #8, D2 
    LSR.L #1, D2 
    OR.B #%00000000, D2
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP)
    JSR PRINT_REGISTER 
    ADDQ.L #4, D2 
    JSR PRINT_COMMA 
    
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.B #SUB_REG_M, D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE

SUB_TO_D:   * DN - <EA> -> DN
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
* MASKING TO GET THE EFFECTIVE ADDRESS
    AND.B #SUB_REG_M, D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    JSR PRINT_COMMA 
* MASKING TO GET THE DATA REGISTER 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #SUB_DREG_M, D2 
    LSR.W #8, D2 
    LSR.W #1, D2 
* APPEND MODE TO D2 FOR PRINT_REGISTER
    OR.B #%00000000, D2 
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE
SUB_END:
    RTS

SUBQ: 
SUBQ_PATTERN  EQU  %0101000100000000
SUBQ_MASKING  EQU  %1111000100000000
SUBQ_REGISTER EQU  %00111111
SUBQ_SIZE     EQU  %11000000
SUBQ_DATA     EQU  %0000111000000000
    MOVE.W D1, D2 
    ANDI.W #SUBQ_MASKING, D2 
    CMPI.W #SUBQ_PATTERN, D2 
    BNE END_SUBQ
    ; CHECK IF THE REGISTER IS SUPPORTED IN THIS PROJECT 
    MOVE.B D1,D2 
    AND.B #SUBQ_REGISTER, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ.L #2, SP 
    MOVE.W (RETURN_ADDRESS), D2 
    CMPI.W #C_UNSUPPORT_REGISTER, D2 
    BEQ FINISH_OPCODE
    ; EVERYTHING IS OK 
    JSR PRINT_SUBQ
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    ANDI.B #SUBQ_SIZE, D2 
    LSR.B #6, D2 
    MOVE.B D2, D6 ; save for print register
    MOVE.B D2, -(SP)
    JSR PRINT_DATA 
    ADDQ.L #2, SP 
    JSR PRINT_TAB 
    JSR PRINT_HASH_SIGN
    JSR PRINT_DOLLAR_SIGN 
    MOVE.W (OPCODE_ADDRESS), D1 
    ANDI.W #SUBQ_DATA, D1
    LSR.W #8, D1
    LSR.B #1, D1
    ADD.B #48, D1
    ; convert to ASCII to print 
    MOVE.B #6, D0 
    TRAP #15 
    JSR PRINT_COMMA
    MOVE.W (OPCODE_ADDRESS), D1 
    ANDI.B #SUBQ_REGISTER, D1
    MOVE.W D1, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    BRA FINISH_OPCODE
END_SUBQ:
    RTS

DIVS:
* CONSTANT FOR DIVS OPCODE
DIVS_OPCODE EQU %1000000111000000
DIVS_M      EQU %1111000111000000
DIVS_DREG_M EQU %0000111000000000
DIVS_REG_M  EQU %0000000000111111

    MOVE.W D1, D2
* MASKING WITH DIVS UNIQUE CODE
    AND.W #DIVS_M, D2
    CMPI.W #DIVS_OPCODE, D2
    BNE DIVS_END

* CHECK TO MAKE SURE WE SUPPORT THE EA
    MOVE.W D1, D2 
    AND.B #DIVS_REG_M, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ #2, SP 
* from print_register to see if it is unsupported register 
    MOVE.W (RETURN_ADDRESS), D2 
    CMPI.W #C_UNSUPPORT_REGISTER, D2 ; IF EQUAL RETURN TO FETCH NEXT OPCODE 
    BEQ FINISH_OPCODE
    JSR PRINT_DIVS
    MOVE.W (OPCODE_ADDRESS), D1
* GET SIZE OF DIVS
    MOVE.B #$01, D6
    MOVE.B D6, -(SP)
    JSR PRINT_DATA
    ADDQ.L #$02, SP 
    JSR PRINT_TAB

* DN - <EA> -> DN
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
* MASKING TO GET THE EFFECTIVE ADDRESS
    AND.B #DIVS_REG_M, D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    JSR PRINT_COMMA 
* MASKING TO GET THE DATA REGISTER 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #DIVS_DREG_M, D2 
    LSR.W #8, D2 
    LSR.W #1, D2 
* APPEND MODE TO D2 FOR PRINT_REGISTER
    OR.B #%00000000, D2 
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE
DIVS_END:
    RTS    

MULS:
* CONSTANT FOR MULS OPCODE
MULS_OPCODE EQU %1100000111000000
MULS_M      EQU %1111000111000000
MULS_DREG_M EQU %0000111000000000
MULS_REG_M  EQU %0000000000111111

    MOVE.W D1, D2
* MASKING WITH MULS UNIQUE CODE
    AND.W #MULS_M, D2
    CMPI.W #MULS_OPCODE, D2
    BNE MULS_END

* CHECK TO MAKE SURE WE SUPPORT THE EA
    MOVE.W D1, D2 
    AND.B #MULS_REG_M, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ #2, SP 
* from print_register to see if it is unsupported register 
    MOVE.W (RETURN_ADDRESS), D2 
    CMPI.W #C_UNSUPPORT_REGISTER, D2 ; IF EQUAL RETURN TO FETCH NEXT OPCODE 
    BEQ FINISH_OPCODE
    JSR PRINT_MULS
    MOVE.W (OPCODE_ADDRESS), D1
* GET SIZE OF MULS
    MOVE.B #$01, D6
    MOVE.B D6, -(SP)
    JSR PRINT_DATA
    ADDQ.L #$02, SP 
    JSR PRINT_TAB

* DN - <EA> -> DN
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
* MASKING TO GET THE EFFECTIVE ADDRESS
    AND.B #MULS_REG_M, D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    JSR PRINT_COMMA 
* MASKING TO GET THE DATA REGISTER 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #MULS_DREG_M, D2 
    LSR.W #8, D2 
    LSR.W #1, D2 
* APPEND MODE TO D2 FOR PRINT_REGISTER
    OR.B #%00000000, D2 
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE
MULS_END:
    RTS  

CMP:
* CONSTANT FOR CMP OPCODE
CMP_OPCODE      EQU %1011000000000000
CMP_OPCODE1     EQU %1011000001000000
CMP_OPCODE2     EQU %1011000010000000
CMP_M           EQU %1111000111000000
CMP_DREG_M      EQU %0000111000000000
CMP_SIZE_M      EQU %0000000011000000
CMP_REG_M       EQU %0000000000111111

    MOVE.W D1, D2
* MASKING WITH CMP UNIQUE CODE
    AND.W #CMP_M, D2
    CMPI.W #CMP_OPCODE, D2
    BEQ DONE_CHECK_CMP 
    CMPI.W #CMP_OPCODE1, D2 
    BEQ DONE_CHECK_CMP
    CMPI.W #CMP_OPCODE2, D2 
    BEQ DONE_CHECK_CMP
    BRA CMP_END
DONE_CHECK_CMP:

* CHECK TO MAKE SURE WE SUPPORT THE EA
    MOVE.W D1, D2 
    AND.B #CMP_REG_M, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ #2, SP 
* from print_register to see if it is unsupported register 
    * MOVE.W (RETURN_ADDRESS), D2 
    * CMPI.W #C_UNSUPPORT_REGISTER, D2 ; IF EQUAL RETURN TO FETCH NEXT OPCODE 
    * BEQ FINISH_OPCODE
    JSR PRINT_CMP
    MOVE.W (OPCODE_ADDRESS), D1
    MOVE.W D1, D2
* MASKING TO GET SIZE OF SUB
    AND.W #SUB_SIZE_M, D2
    ASR.W #6, D2
    MOVE.B D2, D6
    MOVE.B D2, -(SP)
    JSR PRINT_DATA
    ADDQ.L #$02, SP 
    JSR PRINT_TAB

* DN - <EA> -> DN
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
* MASKING TO GET THE EFFECTIVE ADDRESS
    AND.B #CMP_REG_M, D2 
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    JSR PRINT_COMMA 
* MASKING TO GET THE DATA REGISTER 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    AND.W #CMP_DREG_M, D2 
    LSR.W #8, D2 
    LSR.W #1, D2 
* APPEND MODE TO D2 FOR PRINT_REGISTER
    OR.B #%00000000, D2 
    MOVE.W D2, -(SP)
    MOVE.W #$0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE
CMP_END:
    RTS  
CMPI: 
CMPI_PATTERN  EQU    %0000110000000000
CMPI_MASK     EQU    %1111111100000000
CMPI_SIZE      EQU   %11000000
CMPI_REGISTER EQU    %00111111
    MOVE.W D1, D2 
    ANDI.W #CMPI_MASK, D2 
    CMPI.W #CMPI_PATTERN, D2 
    BNE CMPI_END
    ; mask to make sure we support the register
    MOVE.B D1,D2 
    ANDI.B #CMPI_REGISTER, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ #2, SP 
    JSR PRINT_CMPI  
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.B D1,D2 
    ANDI.B #CMPI_SIZE, D2 
    LSR.B #6, D2 
    MOVE.B D2, D6 ; SAVE FOR LATER 
    MOVE.B D2, -(SP)
    JSR PRINT_DATA 
    ADDQ.L #2, SP 
    JSR PRINT_TAB
    ; print immediate value 
    MOVE.B #IMMEDATE_VALUE, D2
    MOVE.W D2, -(SP)
    MOVE.W D6, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    ; print effective register
    JSR PRINT_COMMA
    MOVE.W (OPCODE_ADDRESS), D1 
    ANDI.B #CMPI_REGISTER, D1 
    MOVE.W D1, -(SP)
    MOVE.W #0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP
    BRA FINISH_OPCODE
CMPI_END:
    RTS


* Branches are located here 
BCC: 
BCC_PATTERN EQU      %0110000000000000
BCC_MASKING EQU      %1111000000000000
BCC_CONDITION EQU    %0000111100000000
BCC_DISPLACEMENT EQU %11111111
BCC_BCS EQU           %0101
BCC_BGE EQU           %1100
BCC_BLT EQU           %1101
BCC_BVC EQU           %1000
* Bcc (BCS, BGE, BLT, BVC)
    MOVE.W D1, D2 
    ANDI.W #BCC_MASKING, D2
    CMPI.W #BCC_PATTERN, D2 
    BNE BCC_DISPLACEMENT_END
    MOVE.W D1, D2 
    ANDI.W #BCC_CONDITION, D2 
    LSR.W #8, D2 
    CMP.B #BCC_BCS, D2 
    BEQ PRINT_BCS
    CMP.B #BCC_BGE, D2
    BEQ PRINT_BGE
    CMP.B #BCC_BLT, D2 
    BEQ PRINT_BLT
    CMP.B #BCC_BVC, D2 
    BEQ PRINT_BVC
    RTS
DONE_PRINT_BCC:
    MOVE.B D1, D2 
    ANDI.B #BCC_DISPLACEMENT, D2 
    MOVE.B D2 , -(SP)
    JSR PRINT_DISPLACEMENT
    ADDQ.L #2, -(SP)
    BRA FINISH_OPCODE
BCC_DISPLACEMENT_END:
    RTS

BCLR: 
BCLR_PATTERN EQU        %0000100010000000
BCLR_PATTERN_DATA EQU   %0000000110000000
BCLR_MASKING EQU        %1111111111000000
BCLR_MASKING_D EQU      %1111000111000000
BCLR_REGISTER EQU       %00111111
BCLR_SIZE_MASK EQU      %00111000
BCLR_DN EQU             %0000111000000000
    MOVE.W D1,D2 
    ANDI.W #BCLR_MASKING, D2 
    CMPI.W #BCLR_PATTERN, D2 
    BEQ BCLR_CHECK_REGISTER
    MOVE.W D1, D2 
    ANDI.W #BCLR_MASKING_D, D2 
    CMPI.W #BCLR_PATTERN_DATA, D2 
    BEQ BCLR_CHECK_REGISTER
    BNE BCLR_END
BCLR_CHECK_REGISTER:
    ;check register 
    ANDI.B #BCLR_REGISTER, D1 
    MOVE.W D1, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ.L #2, SP 
    JSR PRINT_BCLR
    ; GET SIZE 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.B D1,D2 
    ANDI.B #BCLR_SIZE_MASK, D2 
    ; DN IS ZERO IN MODE
    CMPI.B #0, D2 
    BEQ BCLR_LONG
    MOVE.B #0, -(SP)
    JSR PRINT_DATA 
    ADDQ.L #2, SP 
BCLR_PRINTED_SIZE:
    ; PRINT IMMEDIATE DATA OR PRINT_DATA REGISTER
    JSR PRINT_TAB 
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    ANDI.W #BCLR_MASKING, D2 
    CMPI.W #BCLR_PATTERN, D2
    BEQ BCLR_PRINT_IM
    BRA BCLR_PRINT_DATA
;PRINT_IMMEDIATE VALUE
BCLR_PRINT_IM:
    ; push data to stack for print register
    MOVE.W #%00111100, -(SP)
    ; because it is always long field, print long 
    MOVE.W #1, -(SP)
    BRA BCLR_PRINT_SOURCE
;PRINT REGISTER DATA
BCLR_PRINT_DATA:
    MOVE.W D1, D2 
    ANDI.W #BCLR_DN, D2 
    MOVE.W D2, -(SP)
    MOVE.W #0, -(SP)
BCLR_PRINT_SOURCE:
    LSR.W #8, D2 
    LSR.B #1, D2 
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    JSR PRINT_COMMA
    ; PRINT EFFECTIVE REGISTER
    MOVE.W (OPCODE_ADDRESS), D1 
    ANDI.B #BCLR_REGISTER, D1 
    MOVE.W D1, -(SP)
    MOVE.W #0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
    BRA FINISH_OPCODE 

BCLR_LONG:
    ; print_out long
    MOVE.B #2, -(SP)
    JSR PRINT_DATA
    ADDQ.L #2, SP 
    BRA BCLR_PRINTED_SIZE

BCLR_END:
    RTS


LSD_REGISTER: 



ALRSD_DATA:
LSD_DATA_PATTERN EQU  %1110000000001000
ASD_DATA_PATTERN EQU  %1110000000000000
ROD_DATA_PATTERN EQU  %1110000000011000
ALRSD_DATA_MASK EQU     %1111000000011000
ALRSD_DATA_SIZE   EQU   %0000000011000000
ALRSD_DATA_DIR    EQU   %0000000100000000
ALRSD_DATA_CR     EQU   %00100000
ALRSD_DATA_CRCOUNT  EQU   %00000000
ALRSD_DATA_REGISTER EQU %00100000 
ALRSD_DATA_CR_VALUE EQU %0000111000000000
ALRSD_DATA_REGISTER_M EQU %00000111
ALRSD_DATA_TYPE        EQU %00011000


    MOVE.W D1,D2 
    ANDI.W #ALRSD_DATA_MASK, D2 
    CMPI.W #LSD_DATA_PATTERN, D2 
    BEQ ALRSD_MATCH_DATA
    CMPI.W #ASD_DATA_PATTERN, D2 
    BEQ ALRSD_MATCH_DATA
    CMPI.W #ROD_DATA_PATTERN, D2 
    BEQ ALRSD_MATCH_DATA
    BRA END_LSD_ASD_DATA

    ; get direction 
ALRSD_MATCH_DATA:

    MOVE.W D1,D0
    ANDI.W #ALRSD_DATA_TYPE, D0
    LSR.B #3, D0
    JSR PRINT_LSD
    JSR PRINT_ASD 
    JSR PRINT_ROD
    MOVE.W D1, D0
    ANDI.W #ALRSD_DATA_DIR, D0
    LSR.W #8, D0
    JSR PRINT_R
    JSR PRINT_L
DONE_PRINT_LSD_D:
    ; print size 
    MOVE.W D1, D2 
    ANDI.W #ALRSD_DATA_SIZE,D2 
    LSR.W #6, D2 
    MOVE.B D2, -(SP)
    JSR PRINT_DATA 
    ADDQ.L #2, SP 
    ; print tab
    JSR PRINT_TAB
    MOVE.W (OPCODE_ADDRESS), D1 
    MOVE.W D1, D2 
    ; get count or register 
    ANDI.B #ALRSD_DATA_CR, D2 
    CMPI.B #ALRSD_DATA_CRCOUNT, D2 
    BEQ LSD_DATA_COUNT 
    BRA LSD_DATA_R 
; is it count
LSD_DATA_COUNT:
    MOVE.W D1, D2 
    ANDI.W #ALRSD_DATA_CR_VALUE, D2 
    LSR.W #8, D2 
    LSR.B #1, D2 
    MOVE.B D2,D1 
    CMPI.B #0, D2 
    BEQ LSD_DATA_8
    BRA LSD_DATA_PRINT_VAL
; is it 8 bits
LSD_DATA_8:
    MOVE.B #8, D1 
; let print count value
LSD_DATA_PRINT_VAl: 
    JSR PRINT_HASH_SIGN
    JSR PRINT_DOLLAR_SIGN
    ADD.B #48, D1 
    MOVE.B #6, D0 
    TRAP #15 
    BRA LSD_DATA_PRINT_REGISTER    ; go to print data register for destination
LSD_DATA_R: ; if it is not a count value
    MOVE.W D1, D2 
    ANDI.W #ALRSD_DATA_CR_VALUE, D2 
    LSR.W #8, D2 
    LSR.B #1, D2 
    ANDI.B #ALRSD_DATA_REGISTER_M, D2 ; add mode for print register
    MOVE.W D2, -(SP)
    MOVE.W #0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, SP 
LSD_DATA_PRINT_REGISTER: ; print destination
    JSR PRINT_COMMA
    MOVE.W (OPCODE_ADDRESS), D1 
    ANDI.B #ALRSD_DATA_REGISTER_M, D1 
    MOVE.W D1, -(SP)
    MOVE.W #0, -(SP)
    JSR PRINT_REGISTER
    ADDQ.L #4, D1 
    BRA FINISH_OPCODE
END_LSD_ASD_DATA:
    RTS


ALRSD_REGISTER:
ALRSD_RE_MASK    EQU %1111111011000000
ASD_RE_PATTERN  EQU %1110000011000000
LSD_RE_PATTERN  EQU %1110001011000000
ROD_RE_PATTERN  EQU %1110011011000000
ALSD_RE_DIR     EQU %0000000100000000
ALSD_RIGHT      EQU %0000000000000000
ALSD_LEFT       EQU %0000000100000000
ALRSD_REGISTER_M   EQU %00111111
ALRSD_TYPE      EQU %0000111000000000
    MOVE.W D1, D2
    ANDI.W #ALRSD_RE_MASK, D2 
    CMPI.W #ASD_RE_PATTERN, D2 
    BEQ ALRSD_MATCH
    CMPI.W #LSD_RE_PATTERN, D2 
    BEQ ALRSD_MATCH
    CMPI.W #ROD_RE_PATTERN, D2 
    BEQ ALRSD_MATCH
    BRA END_ALSD_ROD_REGISTER
ALRSD_MATCH:
    MOVE.W D1, D2 
    ANDI.B #ALRSD_REGISTER_M, D2 
    MOVE.W D2, -(SP)
    JSR CHECK_SUPPORT_REGISTER
    ADDQ.L #2, SP
    



    MOVE.W D1, D0
    ANDI.W #ALRSD_TYPE, D0
    LSR.W #8, D0
    LSR.B #1, D0
    JSR PRINT_LSD
    JSR PRINT_ASD 
    JSR PRINT_ROD
    MOVE.W D1, D0
    ANDI.W #ALSD_RE_DIR, D0
    LSR.W #8, D0
    JSR PRINT_R
    JSR PRINT_L
ALRSD_DONE_PRINT_R:
    JSR PRINT_TAB
    MOVE.W (OPCODE_ADDRESS), D1 
    ANDI.B #ALRSD_REGISTER_M, D1 
    MOVE.W D1, -(SP)
    MOVE.W #0, -(SP)
    JSR PRINT_REGISTER
    BRA FINISH_OPCODE
END_ALSD_ROD_REGISTER:
    RTS
* ROR:
* * CONSTANT FOR ROR OPCODE
* ROR_OPCODE EQU %
* ROR_M      EQU %
* ROR_SIZE_M EQU %
*     CLR.W D2
*     MOVE.W D1, D2
* * MASKING WITH ROR UNIQUE CODE
*     AND.W #ROR_M, D2
*     CMP.W #ROR_OPCODE, D2
*     BNE ROR_END
*     JSR PRINT_ROR
*     MOVE.W D1, D2
*     MOVE.W D1, D3
* * MASKING TO GET SIZE OF ROR
*     CMP.W #ROR_SIZE_M, D2 
* ROR_END:
*     RTS

* ROL:
* * CONSTANT FROR ROL OPCODE
* ROL_OPCODE EQU %
* ROL_M      EQU %
* ROL_SIZE_M EQU %
*     CLR.W D2
*     MOVE.W D1, D2
* * MASKING WITH ROL UNIQUE CODE
*     AND.W #ROL_M, D2
*     CMP.W #ROL_OPCODE, D2
*     BNE ROL_END
*     JSR PRINT_ROL
*     MOVE.W D1, D2
*     MOVE.W D1, D3
* * MASKING TO GET SIZE OF ROL
*     CMP.W #ROL_SIZE_M, D2 
* ROL_END:
*     RTS


PRINT_RTS:
    LEA P_RTS, A1
    MOVE.B #14, D0
    TRAP #15
    JSR PRINT_TAB
    BRA FINISH_OPCODE
PRINT_NOP:
    LEA P_NOP, A1
    MOVE.B #14, D0
    TRAP #15
    BRA FINISH_OPCODE
PRINT_NEG:
    LEA P_NEG, A1
    MOVE.B #14, D0
    TRAP #15
    RTS 
PRINT_BRA:
    LEA P_BRA, A1
    MOVE.B #14, D0
    TRAP #15
    RTS 
PRINT_OR:
    LEA P_OR, A1
    MOVE.B #14, D0
    TRAP #15
    RTS 
PRINT_ORI:
    LEA P_ORI, A1
    MOVE.B #14, D0
    TRAP #15
    RTS
PRINT_EOR:
    LEA P_EOR, A1
    MOVE.B #14, D0
    TRAP #15
    RTS 
PRINT_JSR:
    LEA P_JSR, A1
    MOVE.B #14, D0
    TRAP #15
    JSR PRINT_TAB
    RTS 
PRINT_LEA:
    LEA P_LEA, A1
    MOVE.B #14, D0
    TRAP #15
    JSR PRINT_TAB
    RTS 
PRINT_ADDA: 
    LEA P_ADDA, A1 
    MOVE.B #14, D0
    TRAP #15 
    RTS
PRINT_ADD:
    LEA P_ADD, A1 
    MOVE.B #14, D0 
    TRAP #15
    RTS
PRINT_SUB:
    LEA P_SUB, A1 
    MOVE.B #14, D0 
    TRAP #15
    RTS
PRINT_SUBQ:
    LEA P_SUBQ, A1 
    MOVE.B #14, D0 
    TRAP #15 
    RTS
PRINT_DIVS:
    LEA P_DIVS, A1 
    MOVE.B #14, D0 
    TRAP #15
    RTS
PRINT_MULS:
    LEA P_MULS, A1 
    MOVE.B #14, D0 
    TRAP #15
    RTS
PRINT_MOVEA: 
    LEA P_MOVEA, A1 
    MOVE.B #14, D0 
    TRAP #15 
    RTS 
PRINT_MOVE: 
    LEA P_MOVE, A1 
    MOVE.B #14, D0 
    TRAP #15 
    RTS 
PRINT_MOVEM: 
    LEA P_MOVEM, A1 
    MOVE.B #14, D0
    TRAP #15
    RTS 
PRINT_UNSUPPORTED: 
    LEA P_DATA, A1 
    MOVE.B #14, D0 
    TRAP #15 
    RTS 

PRINT_CMP:
    LEA P_CMP, A1 
    MOVE.B #14, D0 
    TRAP #15
    RTS
PRINT_CMPI:
    LEA P_CMPI, A1 
    MOVE.B #14, D0 
    TRAP #15
    RTS
PRINT_BCLR: 
    LEA P_BCLR, A1 
    MOVE.B #14, D0 
    TRAP #15 
    RTS

* BCS, BGE, BLT, BVC
PRINT_BCS:
    LEA P_BCS, A1 
    MOVE.B #14, D0 
    TRAP #15 
    BRA DONE_PRINT_BCC
PRINT_BGE:
    LEA P_BGE, A1 
    MOVE.B #14, D0 
    TRAP #15 
    BRA DONE_PRINT_BCC
PRINT_BLT:
    LEA P_BLT, A1 
    MOVE.B #14, D0 
    TRAP #15
    BRA DONE_PRINT_BCC
PRINT_BVC:
    LEA P_BVC, A1 
    MOVE.B #14, D0 
    TRAP #15
    BRA DONE_PRINT_BCC
C_P_ASD EQU $0
C_P_LSD EQU $1 
C_P_ROD EQU $3
PRINT_ASD: 
    CMPI.B #C_P_ASD, D0 
    BNE PRINT_ALRSD_END 
    LEA P_ASD, A1 
    MOVE.B #14, D0 
    TRAP #15 
    RTS
PRINT_LSD: 
    CMPI.B #C_P_LSD, D0 
    BNE PRINT_ALRSD_END 
    LEA P_LSD, A1 
    MOVE.B #14, D0 
    TRAP #15 
    RTS
PRINT_ROD: 
    CMPI.B #C_P_ROD, D0 
    BNE PRINT_ALRSD_END 
    LEA P_ROD, A1 
    MOVE.B #14, D0 
    TRAP #15 
    RTS
PRINT_ALRSD_END:
    RTS
PRINT_R:
    CMPI.B #0, D0 
    BNE PRINT_R_END
    LEA P_R, A1 
    MOVE.B #14, D0 
    TRAP #15 
PRINT_R_END:
    RTS

PRINT_L:
    CMPI.B #1, D0 
    BNE PRINT_L_END
    LEA P_L, A1 
    MOVE.B #14, D0 
    TRAP #15 
PRINT_L_END:
    RTS



P_R     DC.B 'R',0
P_L     DC.B 'L', 0

P_RTS   DC.B 'RTS',0          *-----------------DONE-----------------*
P_NOP   DC.B 'NOP',0          *-----------------DONE-----------------*
P_MOVE  DC.B 'MOVE',0         *-----------------DONE-----------------*
P_MOVEA DC.B 'MOVEA', 0       *-----------------DONE-----------------*
P_MOVEM DC.B 'MOVEM', 0       *-----------------DONE-----------------*
P_ADD   DC.B 'ADD', 0         *-----------------DONE-----------------*
P_ADDA  DC.B 'ADDA',0         *-----------------DONE-----------------*
P_SUB   DC.B 'SUB', 0         *-----------------DONE-----------------*
P_SUBQ  DC.B 'SUBQ',0         *-----------------DONE-----------------* 
P_MULS  DC.B 'MULS',0         *-----------------DONE-----------------*
P_DIVS  DC.B 'DIVS',0         *-----------------DONE-----------------*
P_LEA   DC.B 'LEA',0          *-----------------DONE-----------------*
P_OR    DC.B 'OR',0           *-----------------DONE-----------------*
P_ORI   DC.B 'ORI', 0         *-----------------DONE-----------------*
P_NEG   DC.B 'NEG', 0         *-----------------DONE-----------------*
P_EOR   DC.B 'EOR', 0         *-----------------DONE-----------------*
P_LSD   DC.B 'LS',0           *-----------------DONE-----------------*
P_ASD   DC.B 'AS', 0          *-----------------DONE-----------------*
P_ROD   DC.B 'RO', 0          *-----------------DONE-----------------*
P_BCLR  DC.B 'BCLR',0         *-----------------DONE-----------------*
P_CMP   DC.B 'CMP', 0         *-----------------DONE-----------------*
P_CMPI  DC.B 'CMPI', 0        *-----------------DONE-----------------*
P_BCS   DC.B 'BCS', 0         *-----------------DONE-----------------*
P_BGE   DC.B 'BGE', 0         *-----------------DONE-----------------* 
P_BLT   DC.B 'BLT',0          *-----------------DONE-----------------*  
P_BVC   DC.B 'BVC', 0         *-----------------DONE-----------------* 
P_BRA   DC.B 'BRA',0          *-----------------DONE-----------------*
P_JSR   DC.B 'JSR',0          *-----------------DONE-----------------*
P_DATA  DC.B 'DATA',0         *-----------------DONE-----------------*


WORD_LENGTH EQU $04
LONG_LENGTH EQU $08
LF          EQU $0A
CR          EQU $0D
HT          EQU  $09 

NEW_LINE                DC.B ' ',CR,LF,0
PROMT_INPUT_START       DC.B 'Please enter starting address(CAPITALIZED & NOT AN ODD NUMBER & Its > 3500)',CR,LF 
                        DC.B  '$',0
PROMT_INPUT_END         DC.B 'Please enter ending address(CAPITALIZED & NOT AN ODD NUMBER % Its > 3500)',CR,LF 
                        DC.B '$',0
PROMT_INPUT_CONTINUE    DC.B 'Press enter to continue',CR,LF,0
INVALID_ADDRESS_SGE     DC.B 'Invalid Address, ending should greater than end address', CR,LF,0
INVALID_ADDRESS DC.B 'Invalid address please enter again', CR,LF,0

    SIMHALT             
    
    END    START        ; last line of source




*~Font name~Courier New~
*~Font size~10~
*~Tab type~1~
*~Tab size~4~
